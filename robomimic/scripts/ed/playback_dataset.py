...
from robomimic.utils.vis_utils import apply_filter
from robomimic.scripts.ed.error_detectors import Firework_Failure, Firework_OOD, FireworkCombined, get_obs_at_idx
...
def playback_trajectory_with_obs(
    ...
    global fail_detector, ood_detector, detector
    
    if fail_detector is not None:
        fail_detector.reset()
    ...
    try:
        action_modes_data = traj_grp["intv_labels"][()]
    except:
        action_modes_data = np.array([-1] * len(traj_grp["actions"][()]))

    traj_len = traj_grp["obs/{}".format(image_names[0])].shape[0]
    for i in range(traj_len):
        if video_count % video_skip == 0:
            # concatenate image obs together
            im = [traj_grp["obs/{}".format(k)][i] for k in image_names]
            frame = np.concatenate(im, axis=1)
            obs = get_obs_at_idx(traj_grp['obs'], i)
            action = traj_grp['actions'][i]
            detect_frame = {'obs': obs, 'actions': action}
            is_intv, is_pre_intv, is_ood = False, False, False
            
            if fail_detector is not None:
                is_pre_intv = fail_detector.human_intervene_frame(detect_frame)
                is_intv = fail_detector._value_history["pred_classes"][-1] == 1
            elif ood_detector is not None:
                is_ood = ood_detector.human_intervene_frame(detect_frame)[0]
            else:
                is_intv = action_modes_data[i] == 1
                is_pre_intv = action_modes_data[i] == -10
                
            if is_intv and args.mask_intv:
                frame = apply_filter(frame, color=(0, 255, 0))
            elif is_pre_intv and args.mask_pre_intv:
                frame = apply_filter(frame, color=(255, 0, 0))
            
            if is_ood and args.mask_ood:
                frame = apply_filter(frame, color=(0, 0, 255))
...
def playback_dataset(args):
    ...
    if args.valid:
        demos = f["mask/valid"][:]
        demos = [elem.decode("utf-8") for elem in demos]
        
    if args.fail_detector_path is not None:
        global fail_detector
        fail_detector = Firework_Failure(args.fail_detector_path, 0.8, 3, 2, "idx", eval_idx=slice(7,10), pred_future=args.pred_future, use_prob=True)
    if args.embedding_path is not None:
        global ood_detector
        ood_detector = Firework_OOD(args.ood_ckpt_path, None, args.embedding_path, pred_future=args.pred_future, eval_method="last", dist_metric=args.ood_type, train_kmeans=True)
        
    if fail_detector is not None and ood_detector is not None:
        global detector
        detector = FireworkCombined(ood_detector, fail_detector)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fail_detector_path",
        type=str,
        help="path to the failure detector model",
        default=None,
    )
    ...
    parser.add_argument(
        "--valid",
        action="store_true",
        help="Only render the validation set",
        default=False,
    )
    ...
    parser.add_argument(
        "--mask_intv",
        action="store_true",
        help="Mask the intervention frames",
        default=False,
    )
    ...
    parser.add_argument(
        "--mask_pre_intv",
        action="store_true",
        help="Mask the pre-intervention frames",
        default=False,
    )
    ...
    parser.add_argument(
        "--embedding_path",
        type=str,
        help="path to the ood embedding file",
        default=None,
    )
    ...
    parser.add_argument(
        "--ood_ckpt_path",
        type=str,
        help="path to the ood detector model",
        default=None,
    )
